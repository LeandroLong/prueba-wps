...
OpenLayers.WPS = OpenLayers.Class({
    ...
    parseGetCapabilities: function (resp) {
        ...
        var dom = resp.responseXML ? resp.responseXML : OpenLayers.WPS.Parse.parseXMLString(resp.responseText);
   ...
   //     if (OpenLayers.Util.getBrowserName() == "msie") {
    //        resp.responseXML = null;
     ...
     //   }
     //   var dom = resp.responseXML ? resp.responseXML : OpenLayers.Parse.parseXMLString(text);
    //    this.responseDOM = dom;
    //    try {
   ...
   //     var identifier = OpenLayers.Format.XML.prototype.getElementsByTagNameNS(dom,this.owsNS,  "Identifier")[0].firstChild.nodeValue;
	    ...
	    this.processes[0].outputs[0].setValue(text);
 ...
 */
OpenLayers.WPS.executeRequestTemplate = '<?xml version="1.0" encoding="UTF-8"?>'+
 ...
 * {String} Temple for Execute Request XML 
 */
OpenLayers.WPS.literalOutputTemplate =  '<ows:Identifier>$IDENTIFIER$</ows:Identifier>';
